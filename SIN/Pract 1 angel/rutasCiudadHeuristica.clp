...
;------------------------------------------------------------------------------------
;-------------------- FUNCIÓN HEURÍSTICA h2(n) ---------------------------------------
;-------------------- h2(n) = DistBarrios(n)*min(Cam(n))*Sit(n)----------------------- 
;------------------------------------------------------------------------------------
(deffunction heuristica2 (?posicion_Actual ?posicionFinal ?pieOBici)
	...
	;nuevo
	(if (= coste_cambio_barrio 0) then (bind ?coste_cambio_barrio2 1))
	(if (<> coste_cambio_barrio 0)then (bind ?coste_cambio_barrio2 coste_cambio_barrio))
	(bind ?i2 (member$ ?posicion_Actual ?*minCam*))
	(bind ?CaminoMinimo (nth$ (+ ?i2 1) ?*minCam*))
	
    (bind ?i3 (member$ ?posicionFinal ?*minCam*))
	(bind ?CaminoMinimo2 (nth$ (+ ?i3 1) ?*minCam*)) ;camino minimo posFinal


	(if (eq ?pieOBici  pie) then  (bind ?sit 1))
	(if (eq ?pieOBici  bici)  then  (bind ?sit 0.5))

	(bind ?sumaMins (+ ?CaminoMinimo ?CaminoMinimo2))
	(bind ?sumaMins (/ ?sumaMins 2))
	(bind ?res (* ?sumaMins ?sit))
	(bind ?res (* ?res ?coste_cambio_barrio2))
...
(deffunction inicio ()
	...
	(assert (transporte Q D Q pie coste 0 numOps 0 numPasos 0 PATH Q))
