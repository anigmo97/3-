           ...
           int n, int m)
{
  ...
  for (p = 0; p < pasos; p++) {
  //#pragma omp parallel for private(r,g,b,tot,v)
    for (i = 0; i < n; i++) {
  //#pragma omp parallel for private(r,g,b,tot,v)
      for (j = 0; j < m; j++) {
        r = 0;
        ...
        b = 0;
        tot = 0;
	//#pragma omp parallel for private(v) reduction(+:r,g,b,tot)
        for (k = max(0, i - radio); k <= min(n - 1, i + radio); k++) {
	  // #pragma omp parallel for private(l,v) reduction(+:r,g,b,tot)	  
// #pragma omp parallel for private(v) reduction(+:r,g,b,tot) //mia
