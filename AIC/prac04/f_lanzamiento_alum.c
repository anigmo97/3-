...
#define I_PRED IF_ISS_2.prediccion
    ...
    switch (I_OP) {
        ...
        if (s > FIN_TAMPON_ESCR) return;/* No hay sitio en la estación de reserva */
		...
		/* INSERTAR CÓDIGO */
            ...
            /*** Operando 2 (en Rfp) ***/
            if(Rfp[I_S2].rob == MARCA_NULA){  // Lee valor
                //TE[s].Vk = Regs[I_S2].valor;
                TE[s].Vk = Rfp[I_S2].valor;
                TE[s].Qk = MARCA_NULA;
            } else if( RB[Rfp[I_S2].rob].estado == WB ){  // Lee RB
            TE[s].Vk = RB[Rfp[I_S2].rob].valor;
            TE[s].Qk = MARCA_NULA;
        }else {// Anota entrada del RB
		...
		/* INSERTAR CÓDIGO */
            ...
            RS[s].ocupado = SI;
            RS[s].OP = I_OP;
            RS[s].rob = b; // Enlaza con entrada del RB

		/* INSERTAR CÓDIGO */

            /*** Operando 1 (en Rfp) ***/
            if(Rfp[I_S1].rob == MARCA_NULA){ // Lee valor
