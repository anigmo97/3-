...
<-  ?debug(Mode);
        ...
        if (Length > 0) {
		    +bucle(0);
            -+targeted("false");
            while (targeted("false") & bucle(X) & (X < Length)) {
                ...
                if (Type < 1000) {
                    ...
                    if (Team == 200) {  // Only if I'm AXIS 
                         if (Mode<=2) { 
                             .println("Seeing a dude. . .");
                              }
					    +targeted("true");
                        +targeted_agent(Object);
                    }     
                }
                -+bucle(X+1);
            }
            if (targeted(Objective) & Objective == "true") {
                ?targeted_agent(Agente);
                .nth(6, Agente, NewDestination);
                check_position(NewDestination);
                if(position(valid)){        
                !add_task(task(9999,"TASK_GOTO_POSITION",MyName,NewDestination,""));
                }
                -+state(standing);
            }
