...
+!update_targets 
    ...
    if (C==1){
        ...
        .println("ESQUINA ALCANZADA","\n\n\nCALCULANDO SIGUIENTE ESQUINA ...");
    }
    ...
    */
    ...
    while(position(invalid)) {
        ...
        ?newX(NX2);
        ?newZ(NZ2);
        check_position(pos(NX2, Y, NZ2));
        ?position(P);
        if (Mode<=3) {
            .println("                    COMPROBANDO... (", NX2, ", ", NZ2, ") is ", P);
        }
    }
    ...
    ?newZ(NZ3);
    if (Mode<=3) {
        .println("ME DIRIJO A LA ESQUINA ",N," (",NX3,",",NZ3,")");
        }
    !add_task(task(9999, "TASK_GOTO_POSITION",MyName,pos(NX3,Y,NZ3),""));
    -newX(_);
    -newZ(_);
...
+!init
   <- ?debug(Mode);
    if (Mode<=3) { 
       .println("VOY A INTENTAR IR A LA PRIMERA ESQUINA");
   }
   +contador_esquinas(0);
   +primera_esquina(1);
   .  
